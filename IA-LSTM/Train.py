...
class Performer:
    def __init__(self):
        ...
        # loss and optimizer
        self.ce_lossFunc = nn.CrossEntropyLoss()                       # See favorites
    ...
    def prepare_data(self):
        """
        Initialization data:
             1_ Generate a vocabulary list (including all the words used in this experiment), and find a corresponding index for each word
             2_ Generate embedding matrix, each row of matrix represents a word vector. The corresponding relationship is matrix
             The index of the row corresponding to the word is the same as the index in the vocabulary.
             3_Get the Dataloader of training, validation, and testing
        :return:
        """
        # 1_Build a glossary
        # 1.1_Build a glossary
    ...
    def _train(self):
        ...
        max_val_f1 = 0
        max_val_epoch = 0
        global_step = 0                 #Represents how many mini-batch are trained
        path = None                     #Record the file path of the best model parameters
        # Whole training
        for i_epoch in range(opt.epoch):
            ...
            # Update parameter stage (tarin_dataset)
    ...
    def test(self):
        ...
        print('The test set accuracy rate is：',test_acc)
...
def main():
    if opt.seed is not None:
        ...
        os.environ['PYTHONHASHSEED'] = str(opt.seed)            #Set hash random value
